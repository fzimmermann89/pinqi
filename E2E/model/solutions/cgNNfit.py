...
class _cgNNfit(torch.nn.Module):
    def __init__(
        self,
        q,
        n_data: int = 10,
        ...
        self.q = q
        self.n_data = n_data
        if iterations > 1 and len(initial_lambdas) == 1:
            initial_lambdas = initial_lambdas * iterations
        elif iterations != len(initial_lambdas):
            raise ValueError(f"iterations {iterations} and len(initial_lambdas) {len(initial_lambdas)} do not match")
    ...
    def forward(self, k, enc):
        ...
        x.append(self.solve_problem2(self.q, y[-1]))
...
class cgNNfit(Solution):
    ...
    def pretrain_step(self, batch, opt, sched, batch_idx):
        return self.step(batch, opt, sched, self.nnloss)

    def train_step(self, batch, opt, sched, batch_idx):
        return self.step(batch, opt, sched, self.e2eloss)

    def configure_optimizers(self):
        optim = torch.optim.AdamW(
            [
                {"params": self.net.net.parameters(), "lr": self.hparams.lr_net, "weight_decay": self.hparams.weight_decay},
                {"params": self.net.lambdas.parameters(), "lr": self.hparams.lr_lam, "weight_decay": 1e-3 * self.hparams.weight_decay},
            ]
        )
